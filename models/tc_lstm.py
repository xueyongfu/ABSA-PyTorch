...
class TC_LSTM(nn.Module):
    ...
    def forward(self, inputs):
        ...
        target_embedding = (torch.sum(self.embed(target),dim=1)/target_len).float()
        ...
        # 方法二:拼接
        x_l, x_r = torch.cat((self.embed(x_l),target_embedding),dim=-1), torch.cat((self.embed(x_r),target_embedding),dim=-1)
